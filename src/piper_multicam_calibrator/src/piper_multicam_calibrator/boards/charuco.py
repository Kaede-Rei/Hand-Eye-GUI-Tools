...
class CharucoDetector:
    def __init__(self, config: dict):
        if not hasattr(cv2, "aruco"):
            raise RuntimeError("OpenCV aruco module is unavailable; install opencv-contrib-python or ROS cv2 with aruco")
    ...
    def detect(self, image_bgr, camera_matrix, dist_coeffs) -> BoardObservation:
        ...
        if ids is None or len(ids) == 0:
            return BoardObservation(False, "charuco", "aruco markers not found", image_size=(image_bgr.shape[1], image_bgr.shape[0]), annotated_image=annotated)
        ...
        if charuco_ids is None or ok_count < 4:
            return BoardObservation(False, "charuco", "not enough ChArUco corners", corners_count=int(ok_count or 0), annotated_image=annotated)
        ok, rvec, tvec = cv2.aruco.estimatePoseCharucoBoard(
            charuco_corners, charuco_ids, self.board, camera_matrix, dist_coeffs, None, None
        )
        if not ok:
            return BoardObservation(False, "charuco", "estimatePoseCharucoBoard failed", corners_count=int(ok_count), annotated_image=annotated)
        obj_points = []
        img_points = []
        ...
        if hasattr(cv2, "solvePnPRefineLM"):
            rvec, tvec = cv2.solvePnPRefineLM(obj_points, img_points, camera_matrix, dist_coeffs, rvec, tvec)
        ...
        try:
            cv2.drawFrameAxes(annotated, camera_matrix, dist_coeffs, rvec, tvec, self.square_length_m * 2.0, 2)
        except Exception:
            pass
