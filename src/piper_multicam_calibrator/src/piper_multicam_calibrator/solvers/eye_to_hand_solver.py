...
class EyeToHandKnownBoardSolver:
    ...
    def solve(self, task: CalibrationTask, records: Sequence[SampleRecord]) -> CalibrationResult:
        ...
        if len(valid) < 3:
            raise ValueError(f"eye_to_hand_known_board requires at least 3 valid samples, got {len(valid)}")
        estimates = [
            r.T_base_tool @ self.T_tool_board @ invert_transform(r.cameras[task.camera].T_camera_board)
            for r in valid
        ]
