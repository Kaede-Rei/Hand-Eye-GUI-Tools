...
from piper_multicam_calibrator.core.transform import average_transforms, invert_transform, transform_residual_metrics
...
class CameraToCameraSolver:
    def solve(self, task: CalibrationTask, records: Sequence[SampleRecord]) -> CalibrationResult:
        if not task.reference_camera or not task.target_camera:
            raise ValueError("camera_to_camera task requires reference_camera and target_camera")
        valid = [
            r for r in records
        ...
        if len(valid) < 3:
            raise ValueError(f"camera_to_camera requires at least 3 valid samples, got {len(valid)}")
        estimates = [
            ...
            per_sample.append(
                {
                    ...
                    "reference_reprojection_error_px": record.cameras[task.reference_camera].reprojection_error_px,
                    "target_reprojection_error_px": record.cameras[task.target_camera].reprojection_error_px,
                }
            )
