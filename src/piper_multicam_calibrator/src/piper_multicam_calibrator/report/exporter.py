...
from piper_multicam_calibrator.core.transform import matrix_to_quaternion_xyzw
...
def export_tf_bundle(output_root: Path, project_name: str, results: Iterable[CalibrationResult]) -> Path:
    results = list(results)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out = ensure_dir(output_root / f"{project_name}_tf_{stamp}")
    write_data(out / "c1_tf_tree.yaml", {"transforms": [r.to_payload() for r in results]})
...
def _report_markdown(results) -> str:
    lines = ["# Multi-Camera Calibration Report", ""]
    for r in results:
