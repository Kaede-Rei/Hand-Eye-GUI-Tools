...
class EyeInHandSolver:
    ...
    def solve(
        ...
        if len(valid) < 3:
            raise ValueError(
                f"eye_in_hand requires at least 3 valid samples, got {len(valid)}"
            )
        ...
        t_target2cam = [
            r.cameras[task.camera].T_camera_board[:3, 3].reshape(3, 1) for r in valid
        ]
        ...
        board_in_base = [
            r.T_base_tool @ T_tool_camera @ r.cameras[task.camera].T_camera_board
            for r in valid
        ]
