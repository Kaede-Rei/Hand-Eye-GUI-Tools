...
当前工具覆盖末端相机、外部固定相机和相机间的统一标定链路；采集侧订阅 ROS image / camera_info / TF，求解侧统一使用 `T_A_B` 变换约定，最终输出可直接接入 TF tree 的静态外参，同时包含标定板检测、数据集管理、误差统计和结果复用导出等实用功能
...
│   └── aux_camera_color_optical_frame
└── link_tcp
    └── ee_camera_color_optical_frame
```

| 任务 | 类型 | 输入 | 输出 |
|---|---|---|---|
| `ee_eye_in_hand` | `eye_in_hand` | 末端相机图像、camera_info、任意 `parent -> child` TF | `link_tcp -> ee_camera_color_optical_frame` |
| `fixed_eye_to_hand` | `eye_to_hand` | 外部相机图像、camera_info、任意 `parent -> child` TF，可选 `T_tool_board` | `base_link -> fixed_camera_color_optical_frame` |
| `aux_to_fixed` | `camera_to_camera` | 两路相机同步图像、camera_info、共同标定板检测 | `fixed_camera_color_optical_frame -> aux_camera_color_optical_frame` |
...
| `solvers` | 实现 `eye_in_hand`、`eye_to_hand`、`eye_to_hand_known_board`、`camera_to_camera` |
...
| 相机 | 按配置动态生成相机列表，分别配置图像话题、camera_info、frame_id 和备用内参 |
| 标定板 | 设置 ChessBoard / ChArUco 参数 |
| 机器人 / TF | 检查任意 `parent_frame -> child_frame` TF 是否可查询 |
| 任务 | 选择当前采样和标定任务 |
...
运行 eye-to-hand；若提供 `--t-tool-board-yaml`，使用已知板模式；不提供时会联合估计 `T_tool_board`：

```bash
rosrun hand_eye_calibrator run_calibration.py \
  --config src/hand_eye_calibrator/config/default.yaml \
  --task fixed_eye_to_hand \
...
rosrun hand_eye_calibrator export_static_tf.py \
  outputs/ee_eye_in_hand_*/ee_eye_in_hand.yaml \
  outputs/fixed_eye_to_hand_*/fixed_eye_to_hand.yaml \
  outputs/aux_to_fixed_*/aux_to_fixed.yaml \
